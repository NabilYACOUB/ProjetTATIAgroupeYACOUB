...
df = pd.read_csv("./Tableau_de_classification_d'avis_de_restaurant.csv", nrows=1711,decimal=',')
# nom_colonne_df = ["Avis_rid","Phrase_id","Cible","Categorie","Polarite","FROM","TO","Avis_texte"]
# data = pd.read_csv(r"C:\Users\C\Downloads\Tableau_de_classification_d'avis_de_restaurant.csv",na_values= 'NA', names= nom_colonne_df)
...
# plot_confusion_matrix(text_classifier, X_test, y_test, normalize='true');
...
#     df['Avis_texte'], df['Polarite'], test_size=0.5)
...
# df.target_names = ['negative', 'neutral', 'positive']
...
ESSAI D'AUTRE MODELE QUI N ONT PAS ABOUTI  
CI-DESSOUS  \|/
...
abscissa = [100,200,300,400,500,600,700,800,900,1000]

nb_iteration = np.zeros(10)
...
for i in range(10):
    ...
    clf = MLPClassifier(hidden_layer_sizes=(13,13,13), max_iter=abscissa[i]).fit(X_train, y_train)
    
    predictions = clf.predict(X_test)

    nb_iteration[i] = precision_score(y_test,predictions, average=None).mean()

ax.set_title('accuracy depending on nb of iteration')
ax.plot(abscissa, nb_iteration, marker = 'x')
'''
...
my_revue = ['This Restaurant was so chill', 'Excellent yet not the best', 'Clean and efficient','Wouldn"t recommend']
X_new = vectorizer.fit_transform(my_revue)
